 ...
 */
const blobToBufferOrString = (
  blob: Blob,
  readAs: "string" | "buffer"
 ...
 * @returns ObjectURL
 */
const blobToURL = (text: string): string =>
  URL.createObjectURL(new Blob([text], { type: "text/vtt" }));
/**
 * @param utf8str
 * @returns string
 */
const toVTT = (utf8str: string) =>
  utf8str
    .replace(/\{\\([ibu])\}/g, "</$1>")
    .replace(/\{\\([ibu])1\}/g, "<$1>")
    .replace(/\{([ibu])\}/g, "<$1>")
...
const toWebVTT = async (resource: Blob): Promise<string> => {
  ...
  if (!(resource instanceof Blob)) {
    throw new Error(
      `${moduleName}: Expecting resource to be a Blob but something else found.`
    );
  }
  let text;
